...
program
  ...
  .action(() => {
    ...
    if (!type || type.toLowerCase() === 'js') {
      exec('node .', (error: any, stdout: any, stderr: any) => {
        if (error) {
...
export default {
  ...
  async execute(interaction: ButtonInteraction) {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(currentDirectory, "buttons", `${buttonCustomId}.ts`),
        content
      );
...
program
  ...
  .action(() => {
    ...
    inquirer.prompt(questionsSlash as any).then((answers: any) => {
...
`;
      fs.writeFileSync(
        currentDirectory +
        `/commands/${commandName}.${language.toLowerCase()}`,
        commandFile
      );
...
${messageOn ? "/messages" : ""}
        */`;
        ...
        fs.writeFileSync(
          currentDirectory + "/config.djs.json",
          JSON.stringify(data, null, 4)
        );
        fs.writeFileSync(
          currentDirectory + `/index.${typeLanguage.toLowerCase()}`,
          code
        );
        const {
          MessageFileJS,
        ...
        fs.writeFileSync(
          currentDirectory + `/buttons/djs.${typeLanguage.toLowerCase()}`,
          typeLanguage.toLowerCase() === "js" ? buttonFile : buttonFileTS
        );
        fs.writeFileSync(
          currentDirectory + `/slash_commands/ping.${typeLanguage.toLowerCase()}`,
          typeLanguage.toLowerCase() === "js" ? commandFile : commandFileTS
        );
        fs.writeFileSync(
          currentDirectory + `/messages/helloWorld.${typeLanguage.toLowerCase()}`,
          typeLanguage.toLowerCase() === "js" ? MessageFileJS : MessageFileTS
        );
...
async function createSampleFiles() {
  const description =
    "`The bot ${pingType || 'API'} ping is ${pingType === 'ws' ? interaction.client.ws.ping : apiPing}ms`";
